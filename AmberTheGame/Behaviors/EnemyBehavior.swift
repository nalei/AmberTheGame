/*
 `EnemyBehavior` является подклассом `GKBehavior`, который предоставляет методы для
 удобного построения соответствующих целей и поведения различных вариантов `Enemy`.
 */
...
class EnemyBehavior: GKBehavior {
  ...
  private func addAvoidObstaclesGoal(forScene scene: LevelScene) {
    setWeight(2.0, for: GKGoal(toAvoid: scene.polygonObstacles, maxPredictionTime: 1.0))
  }
